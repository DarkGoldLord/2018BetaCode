...
#include <mechanisms/CubeHandlerFactory.h>

#include <subsys/Grabber.h>
#include <subsys/Lift.h>

CubeToFloor::CubeToFloor() : m_controller( TeleopControl::GetInstance() ),
							 m_lift( nullptr )
{
	...
	{
		printf( "CubeToFloor has no controller \n" );
	}

	if ( m_lift == nullptr )
	{
		printf( "CubeToFloor has no lift mechanism \n" );
	}
}
...
// Returns:         void
//-----------------------------------------------------------------------------------
ICubeHandler::CURRENT_CUBEHANDLER_STATE CubeToFloor::Periodic()
{
	ICubeHandler::CURRENT_CUBEHANDLER_STATE state = ICubeHandler::MOVE_TO_FLOOR_POS;
