...
//        double maxAngle = Servo::GetMaxAngle();
        ...
        for (pugi::xml_attribute attr = ServoNode.first_attribute(); attr; attr = attr.next_attribute())
        {
            if ( strcmp( attr.name(), "usage" ) == 0 )
            {
                ...
                switch ( iVal )
                {
                    case DragonServo::SERVO_SIDEBAR_RELEASE_1:
                        usage = DragonServo::SERVO_SIDEBAR_RELEASE_1;
                        break;

                    case DragonServo::SERVO_SIDEBAR_RELEASE_2:
                        usage = DragonServo::SERVO_SIDEBAR_RELEASE_2;
                        break;
