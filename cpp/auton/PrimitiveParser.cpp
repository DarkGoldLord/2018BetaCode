...
)
{
    ...
    float                       xloc = 0.0;
    float                       yloc = 0.0;
    Lift::LIFT_PRESET_HEIGHTS   liftPosition = Lift::KEEP_SAME;
            ...
            for (pugi::xml_node primitiveNode = node.first_child(); primitiveNode; primitiveNode = primitiveNode.next_sibling())
            {
                if ( strcmp( primitiveNode.name(), "primitive") == 0 )
                {
                    ...
                    for (pugi::xml_attribute attr = primitiveNode.first_attribute(); attr; attr = attr.next_attribute())
                    {
                        ...
                        else if ( strcmp( attr.name(), "liftheight" ) == 0 )
                        {
                            int iVal = attr.as_int();
                            switch ( iVal )
                            {
                                ...
                                case Lift::SWITCH:
                                    liftPosition = Lift::SWITCH;
                                    break;

                                case Lift::SCALE:
                                    liftPosition = Lift::SCALE;
                                    break;

                                default:
                                    liftPosition = Lift::KEEP_SAME;
                                    break;
                            }
                        }
                        else if ( strcmp( attr.name(), "opengrabber" ) == 0 )
                        {
                            openGrabber = attr.as_bool();
                        }
                        else if ( strcmp( attr.name(), "cubespeed" ) == 0 )
                        {
                            cubeSpeed = attr.as_float();
                        }
                    }
                    if ( !hasError )
                    {
